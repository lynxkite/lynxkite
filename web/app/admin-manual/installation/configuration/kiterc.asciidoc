...
===========================================================
An application secret will be used by the Play! framework for various tasks, such as signing cookies and
encryption, if the flag below is enabled. More details can be found
https://playframework.com/documentation/latest/ApplicationSecret[here].
```
# export KITE_USE_APPLICATION_SECRET=1
```
