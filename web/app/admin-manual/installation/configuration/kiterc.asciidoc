...
setting `ALLOW_NON_PREFIXED_PATHS` to `true`: then it becomes possible to
specify any valid paths such as `file:/home/user/kite_data/dir/data.csv`
or `s3n://awskey:awspassword@somedir/somefile`. This setting is definitely
not recommended in production!

```
export ALLOW_NON_PREFIXED_PATHS=false
```
...
Set thread stack size for executor threads. By default, jvm will assign 1 megabyte for each thread,
...
```
# export EXECUTOR_THREAD_STACK_SIZE=2M
```
