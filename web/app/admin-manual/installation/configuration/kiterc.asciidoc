...
`KITE_ACCESS_WITHOUT_LOGIN` to `yes`. Whether these users are restricted to seeing wizards
...
The Sphynx server is a very memory hungry process, because it keeps large pieces of data
in memory. This can potentially lead to an out-of-memory situation, where the kernel shuts
Sphynx down. In order to prevent this, the setting below provides a threshold: if the estimated
size of in-memory data exceeds the value provided, Sphynx will keep discarding unused data until the memory
consumption level falls below the threshold.
```
