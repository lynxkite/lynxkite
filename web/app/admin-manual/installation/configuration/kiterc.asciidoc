...
API access to LynxKite is available by sending JSON commands through a Unix pipe.
LynxKite will create this pipe on startup. This pipe gives full administrative access, so use
the file system permissions on the containing directory to limit access to it, if you enable this
option. The API protocol is not documented at the moment but we provide a Python client in
`tools/api/python`.
```
# export KITE_API_PIPE=$HOME/secure/api-pipe
```
