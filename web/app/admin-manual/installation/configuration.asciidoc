...
LynxKite is compatible with HDFS running in Hive Availability mode. In this case the
`KITE_DATA_DIR` variable in <<kiterc-file>> and the <<prefix-definitions>> need to refer to the
appropriate file system defined in the `core-site.xml` configuration file of Hadoop. Please
