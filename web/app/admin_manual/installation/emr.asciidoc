...
There is specialized support for running Kite on EMR. There is a script which takes care of basic cluster
management, deployment and configuration of Kite on the cluster and remote restarts. To use Kite on EMR,
follow these steps:

. Install Kite as explained in (
include::installing-binaries.asciidoc[]
) to a local linux machine.
I will use `INSTALL_DIR` below to refer to the installation directory of Kite.

. Export `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` environment variables with your AWS credentials

. Make a copy of `INSTALL_DIR/tools/emr_spec_template` somewhere. Let’s say your copy’s name is
`my_cluster_spec`.
...
By default, Kite is running behind a firewall to prevent unauthorized access.

. Open a connection to your Kite instance:
`INSTALL_DIR/tools/emr.sh connect my_cluster_spec`
Now you can access your Kite instance by entering http://localhost:4044 in your
browser. If you want to access YARN and Spark diagnostic tools, or more Kite instances
...
created by Kite needs to be backed up.
...
list of projects and the history of executed operations in the projects.
  ...
  .. Make the copy of the data to S3 (this only works if S3_DATAREPO is configured properly):
