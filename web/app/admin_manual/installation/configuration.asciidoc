...
- NameNode and DataNode directoires are better to not exist or at least be empty before you start
...
- Set the `yarn.nodemanager.local-dirs` (YARN app cache and file cache) to be on the appropriate
  drive. It should be sufficiently large (typically at least 100GB). It may also make sense to put
  it on an SSD to make LynxKite faster.
