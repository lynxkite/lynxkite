...
the following filter matches the empty strings: `regex(^$)`.
...
are is a way to accomplish this. It relies on operation <<fill-with-constant-default-value>>,
which will change any undefined value to whatever is specified in the operation's parameter.
The drawback of this method is that the undefined values will no longer be undefined (although
the vertices/edges will still be identifiable by their default value if that value did
not occur elsewhere). This problem can be avoided if you make a copy of the original
attribute before launching the operation.
...
Unfortunately, importing this data from a CSV file will treat undefined values
...
`attr == 0 ? undefined :  attr2`

will return attr2 whenever the value of attr is nonzero, and undefined
otherwise. This way, you can create another attribute (say attr3), which will be
either equal to attr2 or undefined.
