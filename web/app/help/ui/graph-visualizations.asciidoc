...
below).
You can access more advanced settings by clicking at
...
Restrictions from filters:: Discards current restrictions and replaces them with a copy of the
currently set vertex filters. Afterwards, the restrictions can be freely modified here.
...
non-positive value, then an extra bucket will be created which will contain all non-positive values.
