...
A "graph box output" represents the base graph and its segmentations in one bundle. The popup
for a graph output shows basic information about the base graph, such as the number of vertices and
...
The <<Graph box outputs>> chapter gives a more in-depth description of graph box outputs.
