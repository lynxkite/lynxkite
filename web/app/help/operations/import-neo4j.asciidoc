...
The label of the node that you want to import from Neo4j. The node will be imported as a table, with
each property as a column. You can specify the properties to import using the `Columns to import`
parameter. The id ( id() function of Neo4j) of the node will be automaticly included
...
both the source and the destination nodes, will be automaticly included in the import as the special
...
`Double`. If disabled, all columns are imported as ``String``. It is recommended to set this to false,
