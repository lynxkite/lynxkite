...
[[sql]] SQL::
Spark SQL query to apply to the data after importing it. The imported "raw" table
can be referred as `this` in the query:
`SELECT <columns> FROM this WHERE <condition> ...`.
