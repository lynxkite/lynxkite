...
- **Overwrite from the table**: Where the table defines new values, those will be used.
...
- **They must match**: An assertion is made to ensure that the values in the table
  are identical to the values in the graph for each vertex.
