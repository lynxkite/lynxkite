...
attributes (such as calculating <<PageRank>>), changing the graph structure (such as
...
can be accessed via
the +++<div class="operation-toolbox" style="display: inline-block;"><div class="categories"><div class="category yellow"><i class="glyphicon glyphicon-search"></i></div></div></div>+++ icon.
This is usually the fastest way to access the desired operation.
...
Operations in this category perform their task without looking at other
edges or vertices. They are not available if the graph has no vertices.
For example, <<add-constant-vertex-attribute>>.
...
For example, <<add-random-edge-attribute>>. (Note that there is one operation in this category which can
actually look at other vertices: <<derived-edge-attribute>> can use the `src$` and `dst$` notation
to access attributes of the source and destination vertices, but this is the only exception.)
...
These operations create <<segmentations>>. For example <<connected-components>>.
...
These operations import external data to LynxKite. Examples: <<import-edge-attributes>>.
...
Three operations belong to this category:
<<fingerprinting-based-on-attributes>>,
<<fingerprinting-between-project-and-segmentation>>, and
<<viral-modeling>>.
...
discarding vertices and/or edges. Examples: <<add-reversed-edges>>, <<new-vertex-set>>.
...
include::internal-edge-id-as-attribute.asciidoc[]
...
include::vertex-attributes-to-position.asciidoc[]
