...
Position::
Available on attributes of type `(number, number)`. The attribute will be interpreted as (X, Y)
coordinates on the plane and vertices will be laid out on the screen based on these coordinates.
(You can create a `(number, number)` from two number attributes using the
<<Convert vertex attributes to position>> operation.)

Geo coordinates::
Available on attributes of type `(number, number)`. The attribute will be interpreted as a
latitude-longitude coordinate and the vertices will be put on a world map based on this coordinate.
(You can create a `(number, number)` attribute from two number attributes using the
<<Convert vertex attributes to position>> operation.)
