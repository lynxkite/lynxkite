...
* `R-squared` is the coefficient of determination in statisitcs.
* `MAPE` is the mean absolute percentage error, a measure of prediction accuracy in statistics.
* `T-values` are the ratios of the departures of estimated parameters from their notional values to
their standard error. The last T-value is for the intercept. This will not appear for the lasso model.
