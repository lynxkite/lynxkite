...
determination in statisitcs.
* `https://en.wikipedia.org/wiki/Mean_absolute_percentage_error[MAPE]` is the mean absolute percentage
error, a measure of prediction accuracy in statistics.
* `https://en.wikipedia.org/wiki/T-statistic[T-values]` can be used in hypothesis testing. The last T-value
is for the intercept. This will not appear for the lasso model.
