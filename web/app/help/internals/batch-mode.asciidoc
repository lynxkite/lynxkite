...
namespace.
 ...
 - `GroovyProject lynx.project(String name, Boolean reset = false)` will return the given project,
   creating it if it does not yet exist. Setting the optional reset argument to true will reset the
   project to an empty state before returning it.
...
The properties of `GroovyProject` are:
 ...
 - `Map<String, GroovyScalar> scalars` is the list of scalars. `GroovyScalar` has a `toString()`
   and a `toDouble()` method, they returns its value as a string/double respectively.
