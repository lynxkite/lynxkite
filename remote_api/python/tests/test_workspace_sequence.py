...
class TestWorkspaceSequence(unittest.TestCase):
  ...
  def test_one_date(self):
    ...
    table_raw = cnt_result_tss.read_interval(lk, test_date, test_date).get_table_sample(lk)
    self.assertEqual(table_raw.data[0][0].string, '4')
    d_result_tss = wss.output_sequences()['d']
    table_raw = d_result_tss.read_interval(lk, test_date, test_date).get_table_sample(lk)
