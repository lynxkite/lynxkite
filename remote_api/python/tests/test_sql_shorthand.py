...
class TestSQLShorthand(unittest.TestCase):
  ...
  def test_sql_with_three_inputs(self):
    lk = lynx.kite.LynxKite()
    t1 = lk.createExampleGraph().sql(sql='select name, age from vertices')
    t2 = lk.createExampleGraph().sql(sql='select age, income from vertices where age <30')
    t3 = lk.createExampleGraph().sql(sql='select income, name from vertices')

    res = lk.sql(t1, t2, t3,
                 sql='''select one.name, two.age, three.income
                 from one, two, three
  ...
  def test_sql1_on_state(self):
    lk = lynx.kite.LynxKite()
    t = lk.createExampleGraph().sql(sql='select name from vertices where age < 20')
