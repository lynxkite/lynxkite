...
class TestSQLShorthand(unittest.TestCase):
  ...
  def test_sql1_on_state(self):
    lk = lynx.kite.LynxKite()
    table = lk.createExampleGraph().sql('select name from vertices where age < 20').get_table_sample()
  ...
  def test_limit(self):
    lk = lynx.kite.LynxKite()
    table = lk.createVertices(size=15).sql('select * from vertices').get_table_sample()
    self.assertEqual(len(table.data), 15)
    small_table = lk.createExampleGraph().sql('select * from vertices').get_table_sample(limit=2)
    self.assertEqual(len(small_table.data), 2)
    eg_table = lk.createExampleGraph().sql('select * from vertices').get_table_sample(limit=8)
