...
class TestViewSchema(unittest.TestCase):
  ...
  def test_schema(self):
    ...
    schema_response = self.lk.sql('''SELECT false AS zero, 1 AS one, 2.0 AS two, 'three' AS three''').schema()
    self.assertEqual(self.get_column_names(self, schema_response), ['zero', 'one', 'two', 'three'])
    self.assertEqual(self.get_data_types(self, schema_response), ['BooleanType', 'IntegerType', 'DoubleType', 'StringType'])
