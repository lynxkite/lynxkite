...
class ProjectOperations(env: SparkFreeEnvironment) extends OperationRegistry {
  ...
  register("Load snapshot", StructureOperations)(new ProjectOutputOperation(_) {
    override val params = super.params + Param("path", "Path")
    def enabled = FEStatus.enabled
    def apply() = {
  ...
  private def applyMergeParallelEdges(
    project: ProjectEditor, params: LazyParameters, byKey: Boolean) = {
  ...
  register("Merge parallel edges", StructureOperations, new ProjectTransformation(_) {
    override val params = super.params ++ aggregateParams(project.edgeAttributes)
    def enabled = project.hasEdgeBundle
  ...
  register("Aggregate vertex attribute globally", GlobalOperations, new ProjectTransformation(_) {
    override val params = super.params + Param("prefix", "Generated name prefix") ++
  ...
  private def getShapeFilePath(params: LazyParameters): String = {
  ...
  def parseAggregateParams(params: LazyParameters) = {
