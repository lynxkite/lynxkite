...
class ProjectOperations(env: SparkFreeEnvironment) extends OperationRegistry {
  ...
  trait SegOp extends ProjectOperation {
  ...
  register("Import edges for existing vertices", ImportOperations, new ProjectTransformation(_) {
    def parameters = List(
      TableParam(
        "table",
        "Table to import from",
  ...
  register("Take segmentation as base project", StructureOperations, new ProjectTransformation(_) {
    def parameters = List(
      Choice("segmentation", "Segmentation to be set as base project", options = segmentations)
    )
    def enabled = hasSegmentation
    def apply() = {
  ...
  register("Segment by event sequence", CreateSegmentationOperations, new ProjectTransformation(_) {
    ...
    def parameters = List(
      ...
      Choice("time_attr", "Time attribute", options = vertexAttributes[Double]),
