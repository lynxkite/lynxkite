...
class Operations(env: SparkFreeEnvironment) extends OperationRepository(env) {
  ...
  register("Random walk sample", new StructureOperation(_, _) {
    def parameters = List(
      NonNegInt("startPoints", "Number of starting points", default = 3),
      NonNegInt("walksFromOnePoint", "Number of walks from a starting points", default = 100),
      Ratio("walkAbortionProbability", "Walk length parameter", defaultValue = "0.15"),
