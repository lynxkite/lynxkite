...
object BetterColoring extends OpFromJson {
  ...
  def fromJson(j: JsValue) = BetterColoring()
}
import BetterColoring._
case class BetterColoring()
  ...
  def execute(inputDatas: DataSet,
    ...
    def findColoring() = {
      ...
      println("pertColor with degree ordering has been done for BetterColoring in " + (c1 - start) + " ms")
      updateBestColorSoFar(convexOrdering)
      val c2 = System.currentTimeMillis()
      println("pertColor with convexOrdering has been done for BetterColoring in " + (c2 - c1) + " ms")
      println("This means that BetterColoring finished with the 2 basic orderings in " + (c2 - start) + " ms")
      updateBestColorSoFar(truncateOrdering(bestColoringSoFar, math.round(currentNumberOfColors / 2)))
      val end = System.currentTimeMillis()
      println("truncating has been done for BetterColoring in " + (end - c2) + " ms")
      println("All of it took " + (end - start) + " ms")
      bestColoringSoFar
    }
