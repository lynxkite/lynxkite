...
class ObsolateProject(val projectPath: SymbolPath)(implicit manager: MetaGraphManager) {
  ...
  private def checkpoints_=(cs: Seq[String]): Unit =
    set(rootDir / "checkpoints", cs.mkString(ObsolateProject.separator))
  ...
  def copyCheckpoint(i: Int, destination: ObsolateProject): Unit = manager.synchronized {
  ...
  def copyToSegmentation(to: ObsolateSegmentation): Unit = {
  ...
  abstract class Holder[T <: MetaGraphEntity](dir: SymbolPath) extends Iterable[(String, T)] {
    ...
    def update(name: String, entity: T) = {
      ObsolateProject.validateName(name)
      set(name, entity)
    }
...
object ObsolateProject {
  ...
  def apply(projectPath: SymbolPath)(implicit metaManager: MetaGraphManager): ObsolateProject =
    new ObsolateProject(projectPath)

  def fromPath(stringPath: String)(implicit metaManager: MetaGraphManager): ObsolateProject =
    new ObsolateProject(SymbolPath.parse(stringPath))

  def fromName(name: String)(implicit metaManager: MetaGraphManager): ObsolateProject =
...
case class ObsolateSegmentation(parentPath: SymbolPath, name: String)(
  ...
  def belongsToAttribute: Attribute[Vector[ID]] = {
    ...
    ObsolateProject.withErrorLogging(s"Cannot get 'belongsToAttribute' for $this") {
  ...
  def membersAttribute: Attribute[Vector[ID]] = {
    ...
    val aop = graph_operations.AggregateByEdgeBundle(graph_operations.Aggregator.AsVector[ID]())
    ObsolateProject.withErrorLogging(s"Cannot get 'membersAttribute' for $this") {
