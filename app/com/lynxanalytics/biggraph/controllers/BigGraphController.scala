...
class BigGraphController(val env: BigGraphEnvironment) {
  ...
  def project(user: serving.User, request: ProjectRequest): FEProject = metaManager.synchronized {
    ...
    val categories = ops.categories(context, includeDepricated = false)
  ...
  private def historyStep(
    ...
    val opCategoriesBefore = ops.categories(context, includeDepricated = true)
...
object Operation {
  ...
  case class Category(
      ...
      depricated: Boolean = false) extends Ordered[Category] {
...
abstract class OperationRepository(env: BigGraphEnvironment) {
  ...
  def categories(context: Operation.Context, includeDepricated: Boolean): List[OperationCategory] = {
    val allOps = opsForContext(context) ++ workflowOperations(context)
    val cats = allOps.groupBy(_.category).toList
    cats.filter { x => x._1.visible && (includeDepricated || x._1.depricated == false) }
      .sortBy(_._1).map {
        case (cat, ops) =>
