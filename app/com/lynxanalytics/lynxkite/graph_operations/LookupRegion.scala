...
import com.lynxanalytics.lynxkite.graph_api.DataSet
import com.lynxanalytics.lynxkite.graph_api.OutputBuilder
...
import com.lynxanalytics.lynxkite.graph_util.Shapefile._
...
import com.lynxanalytics.lynxkite.graph_operations.LookupRegion._
...
case class LookupRegion(
  ...
  def execute(
    ...
    val regionAttributeMapping: Seq[(BoundingBox, AnyRef /* Geometry */, String)] =
      sf.iterator.map(feature =>
        (
          ...
          Option(feature.getAttribute(attribute)).map(_.toString()))).filter(_._3.nonEmpty).map { case (b, g, a) =>
        (b, g, a.get)
      }.toVector
    sf.close()
    ...
    output(
      o.attribute,
      inputs.coordinates.rdd.flatMapValues {
        v =>
