...
object TableSpec extends FromJson[TableSpec] {
  ...
  import com.lynxanalytics.lynxkite.serving.FrontendJson.fDataFrameSpec
...
class SQLController(val env: BigGraphEnvironment, ops: OperationRepository) extends play.api.http.HeaderNames {
  ...
  def downloadCSV(table: Table, sampleRows: Int) = {
    ...
    val op = com.lynxanalytics.lynxkite.graph_operations.ExportTableToCSV(
      path = path,
      header = true,
      delimiter = ",",
      quote = "\"",
    ...
    dataManager.getFuture(export).map { _ =>
      ...
      val iter = new com.lynxanalytics.lynxkite.serving.HeaderSkippingStreamIterator(path, streams)
