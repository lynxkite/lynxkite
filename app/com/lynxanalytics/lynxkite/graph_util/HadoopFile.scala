...
import com.lynxanalytics.lynxkite.partitioned_parquet._
...
class HadoopFile private (
  ...
  def readAllowedFrom(user: com.lynxanalytics.lynxkite.serving.User): Boolean = {
    val acl = PrefixRepository.getReadACL(prefixSymbol)
    aclContains(acl, user)
  }

  def writeAllowedFrom(user: com.lynxanalytics.lynxkite.serving.User): Boolean = {
