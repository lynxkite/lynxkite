...
import com.lynxanalytics.lynxkite.graph_operations.ConcatenateBundles
import com.lynxanalytics.lynxkite.graph_operations.AddConstantAttribute

class BundleChain(
    bundles: Seq[EdgeBundle],
  ...
  for (i <- 0 until (bundles.size - 1)) {
    assert(
      bundles(i).dstVertexSet == bundles(i + 1).srcVertexSet,
