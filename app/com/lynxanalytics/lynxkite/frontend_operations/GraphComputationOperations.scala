...
class GraphComputationOperations(env: SparkFreeEnvironment) extends ProjectOperations(env) {
  ...
  register("Approximate clustering coefficient")(new ProjectTransformation(_) {
    ...
    def apply() = {
      ...
      project.newVertexAttribute(
        params("name"),
        op(op.es, project.edgeBundle).result.clustering,
        help)
    }
  })
  ...
  register("Compute centrality")(new ProjectTransformation(_) {
    params ++= List(
      ...
      Choice("weight", "Edge weight", options = FEOption.list("No weighting") ++ project.edgeAttrList[Double]),
      NonNegInt("samples", "Sample size", default = 1000, group = "Advanced settings"),
      NonNegInt("maxDiameter", "Maximal diameter to check", default = 10, group = "Advanced settings"),
    ...
    def apply() = {
      ...
      def nk(algo: String) = graph_operations.NetworKitComputeDoubleAttribute.run(
        algo,
        es,
        Map("samples" -> params("samples").toInt),
        weight)
  ...
  register("Compute diameter")(new ProjectTransformation(_) {
    ...
    def apply() = {
      ...
      val result = graph_operations.NetworKitComputeScalar.run(
        "Diameter",
        project.edgeBundle,
        Map("directed" -> false, "max_error" -> maxError))
      val name = params("name")
      if (maxError == 0) {
  ...
  register("Compute effective diameter")(new ProjectTransformation(_) {
    params ++= List(
      ...
      NonNegInt("approximations", "Number of parallel approximations", default = 64, group = "Esimation settings"),
    )
    def enabled = project.hasEdgeBundle
    def apply() = {
  ...
  register("Compute assortativity")(new ProjectTransformation(_) {
    params ++= List(
      ...
      Choice("attribute", "Attribute", options = FEOption.unset +: project.vertexAttrList[Double]))
    def enabled = project.hasEdgeBundle
    def apply() = {
  ...
  register("Find optimal spanning tree")(new ProjectTransformation(_) {
    params ++= List(
      ...
      Choice("weight", "Edge weight", options = FEOption.list("Unit weight") ++ project.edgeAttrList[Double]),
      Choice("optimize", "Optimize for", options = FEOption.list("Maximal weight", "Minimal weight")),
      ...
      val result = {
        ...
        val clus = {
          ...
          op(op.vs, project.vertexSet)(
            op.es,
            project.edgeBundle).result.clustering
        }
        ...
        op(
          ...
          op.angular,
          angAttr.runtimeSafeCast[Double])(
          op.degree,
          degree)(
          op.clustering,
          clus).result
  ...
  register("Find Steiner tree")(new ProjectTransformation(_) {
    params ++= List(
      ...
      Param("rname", "Output vertex attribute name for the solution root points", defaultValue = "rootpoints"),
      Choice("edge_costs", "Cost attribute", options = project.edgeAttrList[Double]),
      Choice("root_costs", "Cost for using the point as root", options = project.vertexAttrList[Double]),
  ...
  register("Compute distance via shortest path")(new ProjectTransformation(_) {
    ...
    def apply() = {
      ...
      project.newVertexAttribute(
        params("name"),
        op(op.es, project.edgeBundle)(op.edgeDistance, edgeDistance)(
          op.startingDistance,
  ...
  register("Find vertex coloring")(new ProjectTransformation(_) {
    ...
    def apply() = {
      ...
      project.newVertexAttribute(
        params("name"),
        op(op.es, project.edgeBundle).result.coloring,
        help)
    }
  })
  ...
  register("Fingerprint based on attributes")(new ProjectTransformation(_) {
    params ++= List(
      ...
      Choice("weights", "Edge weights", options = FEOption.noWeight +: project.edgeAttrList[Double]),
    ...
    def apply() = {
      ...
      project.newVertexAttribute(
        params("leftName") + " similarity score",
        fingerprinting.leftSimilarities)
      project.newVertexAttribute(
        params("rightName") + " similarity score",
    ...
    def apply() = {
      val result = {
        ...
        val clus = {
          ...
          op(op.vs, project.vertexSet)(
            op.es,
            direction.edgeBundle).result.clustering
        }
