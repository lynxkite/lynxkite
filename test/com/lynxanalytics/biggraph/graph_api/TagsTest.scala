...
class TagsTest extends FunSuite with TestTempDir {
  ...
  test("We can clone a tag") {
    val root = newRoot
    val src = SymbolPath.fromSlashyString("alma/korte/barack")
    val dst = SymbolPath.fromSlashyString("alma/cseresznye")
  ...
  test("We can clone a directory") {
    val root = newRoot
    root.setTag(SymbolPath.fromSlashyString("alma/korte/barack"), "hello")
    root.cp(SymbolPath("alma"), SymbolPath.fromSlashyString("brave_new_world/batoralma"))
    assert((root /
      SymbolPath.fromSlashyString("brave_new_world/batoralma/korte/barack")).content == "hello")
  }
  ...
  test("Directory deletion works") {
    val root = newRoot
    val path = SymbolPath.fromSlashyString("alma/korte/barack")
    root.setTag(path, "hello")
    assert(root.exists(path))
    root.rm(SymbolPath.fromSlashyString("alma/korte"))
    assert(!root.exists(path))
    val root2 = TagRoot(storeFile.toString)
