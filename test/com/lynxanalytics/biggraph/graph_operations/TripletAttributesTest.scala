...
class TripletAttributesTest extends FunSuite with TestGraphOp {
  test("triplets of example graph") {
    ...
    assert(mapped.srcEdges.rdd.collect.toMap.mapValues(_.toSet) == Map(0 -> Set(0), 1 -> Set(1), 2 -> Set(2, 3)))
    assert(mapped.dstEdges.rdd.collect.toMap.mapValues(_.toSet) == Map(0 -> Set(1, 2), 1 -> Set(0, 3)))
