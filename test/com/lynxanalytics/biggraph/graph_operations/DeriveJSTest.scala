...
class DeriveJSTest extends FunSuite with TestGraphOp {
  test("example graph: 'name.length * 10 + age'") {
    ...
    val joined = {
      val op = JoinMoreAttributes(1, 1, 0)
      op(op.numAttrs, Seq(g.age.entity))(op.strAttrs, Seq(g.name.entity)).result.attr
    }
    val op = DeriveJSDouble(
      ...
      vecAttrNames = Seq())
    val derived = op(op.attr, joined).result.attr
  ...
  test("example graph: \"gender == 'Male' ? 'Mr ' + name : 'Ms ' + name\"") {
    ...
    val g = ExampleGraph()().result
    val joined = {
      val op = JoinMoreAttributes(0, 2, 0)
      op(op.numAttrs, Seq())(op.strAttrs, Seq(g.gender.entity, g.name.entity)).result.attr
    }
