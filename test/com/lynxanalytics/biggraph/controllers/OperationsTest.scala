...
class OperationsTest extends FunSuite with TestGraphOp with BigGraphEnvironment {
  ...
  test("Aggregate edge attribute to vertices, all directions") {
    ...
    run("Aggregate edge attribute to vertices", Map(
      ...
      "aggregate-weight" -> "sum"))
    run("Aggregate edge attribute to vertices", Map(
      "prefix" -> "incoming symmetric",
      "direction" -> "incoming symmetric edges",
      "aggregate-weight" -> "sum"))
    run("Aggregate edge attribute to vertices", Map(
      "prefix" -> "outgoing symmetric",
      "direction" -> "outgoing symmetric edges",
      "aggregate-weight" -> "sum"))
    ...
    assert(value("incoming symmetric") == Seq(0L -> 2.0, 1L -> 1.0))
    assert(value("outgoing symmetric") == Seq(0L -> 1.0, 1L -> 2.0))
  }
