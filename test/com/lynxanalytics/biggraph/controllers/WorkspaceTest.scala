...
class WorkspaceTest extends FunSuite with graph_api.TestGraphOp {
  ...
  test("pagerank on example graph") {
    ...
    val project = ws.allStates(ctx)(pr.output("project")).project
  ...
  test("deltas still work") {
    ...
    val project = ws.allStates(ctx)(merge.output("project")).project
    import graph_api.Scripting._
  ...
  test("errors") {
    ...
    val allStates = ws.allStates(ctx)
  ...
  test("2-input operation") {
    using("test-workspace") {
      ...
      val project = ws.allStates(ctx)(combine.output("project")).project
      import graph_api.Scripting._
  ...
  test("progress success") {
    using("test-workspace") {
      ...
      ws.allStates(ctx)(pr.output("project")).project.vertexAttributes(pagerankParams("name"))
        .rdd.values.collect
