...
class DeriveAttributeOperationTest extends OperationsTestBase {
  ...
  test("Multi-line function") {
    ...
    val attr = a.project.vertexAttributes("output").runtimeSafeCast[Double]
  ...
  test("Vector attribute") {
    ...
    val attr = a.project.vertexAttributes("output").runtimeSafeCast[String]
  ...
  ignore("Derive edge attribute") {
    ...
    val attr = a.project.edgeAttributes("tripletke").runtimeSafeCast[String]
  ...
  test("Derive vertex attribute (Vector of Strings)") {
    ...
    val attr = a.project.vertexAttributes("vector").runtimeSafeCast[Vector[String]]
  ...
  test("Derive vertex attribute (Vector of Doubles)") {
    ...
    val attr = a.project.vertexAttributes("vector").runtimeSafeCast[Vector[Double]]
    assert(attr.rdd.collect.toMap == Map(
  ...
  test("Derive vertex attribute (does not return vector)") {
    val e = intercept[org.apache.spark.SparkException] {
      val a = box("Create example graph")
        .box("Derive vertex attribute",
          Map("type" -> "vector of strings", "output" -> "vector", "expr" -> "gender"))
      a.project.vertexAttributes("vector").runtimeSafeCast[Vector[String]].rdd.collect
    }
  ...
  test("Derive vertex attribute (wrong vector generic type)") {
    val e = intercept[org.apache.spark.SparkException] {
      ...
      a.project.vertexAttributes("vector").runtimeSafeCast[Vector[Double]].rdd.collect
    }
    assert(e.getCause.getMessage == "assertion failed: JavaScript([gender]) with values: " +
