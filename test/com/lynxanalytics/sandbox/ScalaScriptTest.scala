...
class ScalaScriptTest extends FunSuite with TestGraphOp {
  ...
  test("Infer type") {
    val t1 = ScalaScript.compileAndGetType(
      "a + b", Map("a" -> typeTag[String], "b" -> typeTag[String]), Map(), paramsToOption = false)
    ...
    val t2 = ScalaScript.compileAndGetType(
      "Vector(a)", Map("a" -> typeTag[String]), Map(), paramsToOption = false)
    ...
    val t3 = ScalaScript.compileAndGetType(
      "a ++ Vector(\"x\")", Map("a" -> typeTag[Vector[String]]), Map(), paramsToOption = false)
  ...
  test("Infer Option type") {
    val ot1 = ScalaScript.compileAndGetType("a", Map(), Map("a" -> typeTag[String]), paramsToOption = true)
    assert(ot1.isOptionType)
    assert(ot1.funcType.tpe =:= typeOf[Option[String] => Option[String]])
    assert(ot1.payloadType =:= typeOf[String])

    val ot2 = ScalaScript.compileAndGetType("Some(1.0)", Map(), Map(), paramsToOption = false)
    ...
    val ot3 = ScalaScript.compileAndGetType(
      "if (a) Some(1.0) else None", Map("a" -> typeTag[Boolean]), Map(), paramsToOption = false)
