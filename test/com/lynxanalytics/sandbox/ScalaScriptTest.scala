...
class ScalaScriptTest extends FunSuite with TestGraphOp {
  ...
  test("Infer type") {
    val t1 = ScalaScript.getType("a + b", Map("a" -> typeTag[String], "b" -> typeTag[String]), toOptionType = false)
    assert(t1.funcType.tpe =:= typeOf[(String, String) => String])
    assert(!t1.isOptionType)
    assert(t1.payLoadType =:= typeOf[String])

    val t2 = ScalaScript.getType("Vector(a)", Map("a" -> typeTag[String]), toOptionType = false)
    ...
    val t3 = ScalaScript.getType("a ++ Vector(\"x\")", Map("a" -> typeTag[Vector[String]]), toOptionType = false)
  ...
  test("Infer Option type") {
    val ot1 = ScalaScript.getType("a", Map("a" -> typeTag[String]), toOptionType = true)
    assert(ot1.isOptionType)
    assert(ot1.funcType.tpe =:= typeOf[Option[String] => Option[String]])
    assert(ot1.payLoadType =:= typeOf[String])

    val ot2 = ScalaScript.getType("Some(1.0)", Map(), toOptionType = false)
    assert(ot2.isOptionType)
    assert(ot2.funcType.tpe =:= typeOf[() => Some[Double]])
    assert(ot2.payLoadType =:= typeOf[Double])

    val ot3 = ScalaScript.getType("if (a) Some(1.0) else None", Map("a" -> typeTag[Boolean]), toOptionType = false)
